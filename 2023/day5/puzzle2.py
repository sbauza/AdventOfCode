...
def main(file):
    ...
    for idx in range(0, len(seeds), 2):
        seeds_map.append((seeds[idx], seeds[idx]+seeds[idx+1]))
    seeds_map = sorted(seeds_map, key=lambda x: x[0])
    print(seeds_map)
    for start, end in seeds_map:
        print(start, end)
        for seed in tqdm(range(start, end)):
            ...
            min_num = min(min_num, src)
    return min_num


assert 46 == main('test.txt')
